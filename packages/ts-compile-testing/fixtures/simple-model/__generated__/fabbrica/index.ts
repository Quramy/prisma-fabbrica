...
type TTraitKeys<TOptions extends UserFactoryDefineOptions> = keyof TOptions["traits"]
...
export interface UserFactoryInterface<TOptions extends UserFactoryDefineOptions> extends UserFactoryInterfaceWithoutTraits {
  traits(name: TTraitKeys<TOptions> | readonly TTraitKeys<TOptions>[]): UserFactoryInterfaceWithoutTraits;
}
...
function defineUserFactoryInternal<TOptions extends UserFactoryDefineOptions>({ defaultData: defaultDataResolver, traits: traitsDefs = { }}: TOptions): UserFactoryInterface<TOptions> {
    const getFactoryWithTraits = (traitKeys: readonly TTraitKeys<TOptions>[] = []) => {
      const seqKey = {};
      ...
      const build = async (inputData: Partial<Prisma.UserCreateInput> = {}) => {
          ...
          const traitsData = traitKeys.reduce(async (queue, traitKey) => {
          ...
          }, Promise.resolve<Partial<UserFactoryDefineInput>>({}))
          const data: Prisma.UserCreateInput = { ...requiredScalarData, ...defaultData, ...defaultAssociations, ...traitsData, ...inputData };
          return data;
      };
    ...
    const traits = (name: TTraitKeys<TOptions> | readonly TTraitKeys<TOptions>[]) => {
      const names = Array.isArray(name) ? [...name] : [name]
      return getFactoryWithTraits(names)
    }
    return {
      ...factory,
      traits
 ...
 *
 * @param options
 * @returns factory {@link UserFactoryInterfaceWithoutTraits}
 */
